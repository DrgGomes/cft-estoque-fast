...
import {
  ...
  Play, Film, GraduationCap, CheckCircle2, Circle, Building2, PaintBucket, ExternalLink, Download, Plug, Send, Box, CheckSquare, Tag, Lock
} from 'lucide-react';
...
export default function App() {
  ...
  const [superAdminAuthenticated, setSuperAdminAuthenticated] = useState(false); // NOVO: Tranca Mestre
  ...
  const academySeasons = useMemo(() => { 
      ...
      return Object.entries(seasonsObj).map(([name, eps]) => ({ 
          name, episodes: eps.sort((a,b) => (a.episode || 0) - (b.episode || 0)) 
  ...
  const generateUpSellerRows = (groupName: string, groupData: any, initialRows: any[] = []) => {
      ...
      groupData.items.forEach((p: Product) => {
          ...
          const safeTitulo = p.name || ''; const finalImages = p.image ? p.image.replace(/,/g, '|') : '';
          rows.push([
              skuPai, p.sku || '', safeTitulo, '', 'N', 'Cor', p.color || '', 'Tamanho', p.size || '', '', '', '', '', '', '', 189.90, p.price || 0, 500, '', p.barcode || '', '', finalImages, p.weight || 800, p.length || 33, p.width || 12, p.height || 19, p.ncm || '', p.cest || '', 'UN', 0, '', p.material || '', p.sole || '', p.fastening || ''
          ]);
      });
      return rows;
  };
  ...
  const handleExportToUpSeller = (groupName: string, groupData: any) => {
      ...
      const worksheet = XLSX.utils.aoa_to_sheet(rows); const workbook = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(workbook, worksheet, "Produtos");
      XLSX.writeFile(workbook, `UpSeller_${groupName.replace(/\s+/g, '_')}.xlsx`); playSound('magic');
  };

  const handleBatchExportToUpSeller = () => {
      if (selectedCatalogGroups.length === 0) return;
      let rows = [UPSELLER_HEADER];
      selectedCatalogGroups.forEach(groupName => { if (groupedProducts[groupName]) { rows = generateUpSellerRows(groupName, groupedProducts[groupName], rows); } });
      const worksheet = XLSX.utils.aoa_to_sheet(rows); const workbook = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(workbook, worksheet, "Produtos_Lote");
      XLSX.writeFile(workbook, `UpSeller_Lote_${selectedCatalogGroups.length}_Modelos.xlsx`); playSound('magic'); setSelectedCatalogGroups([]); 
  };
  ...
  const handleOpenTicket = async (e: React.FormEvent) => { 
      ...
      try { 
          await addDoc(collection(db, getCol('tickets')), { 
              userId: user.uid, userName: userProfile?.name || user.email || 'Revendedor', type: ticketType, status: 'pendente', productId: returnProd.id, productInfo: finalProductInfo, productValue: finalValue, reason: ticketType === 'devolucao' ? ticketReason : 'Troca Normal', createdAt: serverTimestamp() 
          }); 
          setTicketReturnGroup(''); setTicketReturnProductId(''); setTicketDesiredGroup(''); setTicketDesiredProductId(''); setTicketReason(''); 
          alert("Solicitação de Suporte enviada com Sucesso!"); playSound('success'); 
      } catch (error) { console.error(error); alert("Falha ao enviar chamado. Tente novamente."); } finally { setIsSavingBatch(false); } 
  };
  ...
  // TELA 1: SUPER ADMIN (GERENCIADOR DE SAAS)
  if (isSuperAdminMode) {
  ...
  // TELA 3: LOGIN DO INQUILINO (LOJA ESPECÍFICA)
  if (!selectedRole) {
    return (
  ...
  // TELA 4: DASHBOARD DO REVENDEDOR
             ...
             <div>
               {filteredAdminList.length === 0 ? (<p className="text-center text-slate-500 py-10">Nenhum produto encontrado nesse filtro.</p>) : (
                   <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
                       {filteredAdminList.map(([name, group]) => {
                           const firstImage = group.info.image ? group.info.image.split(',')[0] : '';
